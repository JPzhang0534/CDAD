...
def get_args_parser():
    ...
    #---------------------
    parser.add_argument("--input_file", type=str, default='/home/data/zjp/code/bridge/lyf/VGDiffZero-main/VGDiffZero-main/data/refcoco_testa.jsonl', help="input file with expressions and annotations in jsonlines format")
    parser.add_argument("--image_root", type=str, default='/home/data/zjp/dataset/coco2014/images/train2014/',help="path to images (train2014 directory of COCO)")
...
# embs = ['person', 'bicycle', 'car', 'motorbike', 'aeroplane', 'bus', 'train', 'truck', 'boat', 'traffic light',
#         'fire hydrant',
...
if __name__ == '__main__':
    ...
    args.img_list = 'dataset/coco'
    # your data path
    args.data_path = '/home/data/zjp/dataset/coco2014/images'
    ...
    # if args.detector_file:  # 检测文件是检测结果
    ...
    # model = BlipForConditionalGeneration.from_pretrained(blip_path).to(blip_device)
    # prompt_texts = []
    # import json
    ...
    img_open = to_tensor(img)

    # -----------------------------------------------------
    input_latent = encode_imgs(img, vae).to(device)
    noise_sample_num = 1
    ...
    for t in times:
        ...
        _, _ = run_and_display(prompt, controller, latent=latents_noisy, run_baseline=False,
                               generator=g_cpu, t=t, noise_sample_num=noise_sample_num)
        out_atts = []
    ...
    # with torch.no_grad():
        ...
        #         #     boxes = [Box(x=box[0], y=box[1], w=box[2], h=box[3]) for box in
        #         #              detections_map[int(datum["image_id"])]]
        #         #     if len(boxes) == 0:
        ...
        #         boxes = gold_boxes
                ...
                #
                # controller.reset()
                # text_coco = sentence["raw"].lower()
                # prompt = f"a photography of {text_coco}"
                #
                # inputs = processor(img, prompt, return_tensors="pt").to(blip_device)
                # out = model.generate(**inputs)
                # out_prompt = processor.decode(out[0], skip_special_tokens=True)
                #
                # embs_len = len(tokenizer.encode(text_coco)) - 2
                # word_len = len(text_coco.split(" "))
                # # prompt = [prompt + "++" + out_prompt[len(prompt):]]
                # out_prompt = out_prompt.split(" ")
                # last_word = text_coco.split(" ")[-1]
                # out_prompt[2 + word_len] = f"{last_word}++"
                # prompt = [" ".join(out_prompt)]
                # prompt_texts.append(f" {prompt[0]}")
                # print(prompt)
            # imgs = []
            ...
            #                 cross_att = cross_attention_maps[idx][:, :, [3 + embs_len, 5 + embs_len]].mean(2).view(
            #                     res, res).float()
            #             # print(decoder(int(tokenizer.encode(prompt[0])[3+embs_len])),decoder(int(tokenizer.encode(prompt[0])[5+embs_len])))
            #             else:
            #                 cross_att = cross_attention_maps[idx][:, :, [3 + embs_len]].mean(2).view(res,
            #                                                                                          res).float()
            #         except:
            #             cross_att = cross_attention_maps[idx][:, :, [3 + embs_len]].mean(2).view(res, res).float()
            #
            #         if res != 64:
            #             cross_att = F.interpolate(cross_att.unsqueeze(0).unsqueeze(0), size=(64, 64),
            #                                       mode='bilinear', align_corners=False).squeeze().squeeze()
            ...
            #     cam_dict = {}
            #
            #     cam_dict[str(embs.index(text_coco))] = images[i].permute(1, 2, 0).cpu().numpy()[:, :, 0]
        ...
        #     # 调整为 VAE 输入尺寸，编码为 latent 表征。
        #     rgb_512 = F.interpolate(orig_images.permute(0, 3, 1, 2), (512, 512), mode='bilinear',
        #                             align_corners=False).to(device1)
        ...
        #     noise = noise if dtype == torch.float32 else noise.half()
        #     images = []
        #     pred_class_key = data[-1][0][-29:]  # 获取该图像对应的预测目标类别
        #
        #     try:
        ...
        #         continue
        #     decoder = tokenizer.decode
        #     # ------------ Promopt design(BLIP部分) ---------------------
        #     # 为每个类别生成 prompt，并进行图像生成
