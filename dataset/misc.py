...
def collate_fn(raw_batch):
    ...
    # new_array1 = np.array(raw_batch[1])
    # img_mask = torch.tensor(new_array1)
    # img_mask = torch.tensor(raw_batch[1])
    # img_data = NestedTensor(img, img_mask)
    # new_array2 = np.array(raw_batch[2])
    # word_id = torch.tensor(new_array2)
    # word_id = torch.tensor(raw_batch[2])

    # new_array3 = np.array(raw_batch[3])
    # word_mask = torch.tensor(new_array3)
    # # word_mask = torch.tensor(raw_batch[3])
    # text_data = NestedTensor(word_id, word_mask)

    new_array4 = np.array(raw_batch[1])
    bbox = torch.tensor(new_array4)
    # bbox = torch.tensor(raw_batch[4])
    img_path = list(raw_batch[3])
    img_id = list(raw_batch[2])
    phrase = list(raw_batch[4])
    class_img = list(raw_batch[5])
    # batch = [img_data, text_data, bbox, img_path, phrase]
    batch = [img, bbox,img_id,img_path,phrase,class_img]
    return tuple(batch)
