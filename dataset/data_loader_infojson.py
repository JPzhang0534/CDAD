...
from dataset.word_utils import Corpus
...
class TransVGDataset(data.Dataset):
    ...
    def __init__(self, data_root, split_root='data', dataset='referit',
        ...
        self.images = []
        self.data_root = data_root    #/home/data/zjp/dataset/coco2014/
        self.split_root = split_root  # /home/data/zjp/dataset/coco2014/spilt
        self.dataset = dataset        # refcoco
        self.query_len = max_query_len
        self.lstm = lstm
        self.transform = transform
        self.testmode = testmode
        self.split = split            # testA
        ...
        if self.dataset == 'referit':
            ...
            self.im_dir = osp.join(
                self.dataset_root, 'images', 'train2014')    # /home/data/zjp/dataset/images/train2014
            # self.split_dir = osp.join(self.dataset_root, 'val2014')
        ...
        for split in splits:
            ...
            # json 格式
            imgset_file = '{0}_{1}_info.json'.format(self.dataset, split)
            imgset_path = osp.join(dataset_path, imgset_file)
            import json
    ...
    def pull_item(self, idx):
        if self.dataset == 'flickr':
            ...
            img_file, phrases, bbox, category = self.images[idx].values()
        ...
        bbox = bbox.float()
        return img, phrases, bbox,img_path,category
    ...
    def __getitem__(self, idx):
        img, phrases, bbox,img_path, class_img = self.pull_item(idx)
        # phrase = phrase.decode("utf-8").encode().lower()
        # phrase = phrase_o.lower()
        input_dict = {'img': img, 'box': bbox, 'text': phrases}
        ...
        if self.testmode:
            ...
            # print(img.shape)
            return (img,  np.array(bbox, dtype=np.float32), self.images[idx]['file_name'], img_path, phrases, class_img)
