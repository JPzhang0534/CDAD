...
class TransVGDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        img, phrase_o, bbox,img_path,  = self.pull_item(idx)
        # phrase = phrase.decode("utf-8").encode().lower()
        phrase = phrase_o.lower()
        input_dict = {'img': img, 'box': bbox, 'text': phrase_o}
        ...
        if self.testmode:
            ...
            # print(img.shape)
            return (img,  np.array(bbox, dtype=np.float32), self.images[idx][0], img_path, phrase_o,None)
