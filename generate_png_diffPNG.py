    ...
    #     img_path = os.path.join(image_root, file_name)
        ...
        for idx, p in enumerate(sentences):
            ...
            for t in times:
                ...
                model = BlipForConditionalGeneration.from_pretrained(blip_path).to(device1)
                prompt_texts = []

                prompt_blip = f'a photography of {p}'
                raw_image = Image.open(image_path).convert('RGB')
                inputs = processor(raw_image,prompt_blip,return_tensors='pt').to(device1)
        ...
        cv2.imwrite('/home/data/zjp/code/bridge/lyf/VGDiffZero-main/VGDiffZero-main/outputs/attn_db/diffseg/test_dog_noisediffPNG.png',cam_colored)
...
# TODO： 1. 添加CLIP 相关图 融入 Noise 中，另外随机数的选择问题？  2.          3. CLIP_attention map 生成方式  4. 得到map后，如何定位--> bbox
# 测试 Normalization结果 --> Normalization效果更明显
