...
class Attention_keep(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = (attn @ v).transpose(1, 2).reshape(B, N, C)  # clip_surgery
        # x = v.transpose(1, 2).reshape(B, N, C) # mask_clip
        x = self.proj_drop(self.proj(x))   #  这个是否需要？
        x_ori = self.proj_drop(self.proj(x_ori))
        return [x, x_ori]
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        if isinstance(self.attn, Attention) or isinstance(self.attn, Attention_keep):
            x = x.transpose(0, 1)
            x, x_ori = self.attn(x)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.attn == None:
            # apply architecture surgery on the last 6 blocks    原来版本
            # for i in range(1, 14): # surgery 7, maskclip 2    原来是7
            for i in range(1, 8):   # 不含25

                self.attn = Attention(self.embed_dim, self.embed_dim, self.num_heads, True)

                # if i in [1,2,4,5,7,10,11,12,13,14,16,17,19,20,22,23,24]:  #隔两层 #83.6 |       82.5 |
                # if i in [9,10]:  # 隔两层
                # #
                #     self.attn = Attention_keep(self.embed_dim, self.embed_dim, self.num_heads, True)
                # else:
                #     self.attn = Attention(self.embed_dim, self.embed_dim, self.num_heads, True)
        ...
        # output_layers = [5,8,12]
        #
        # output_layers = [3,6,9,12]
        # output_layers = [12]

        output_layers = [8,16,24,32]   # vit-L/14
        # output_layers = [8, 12, 18, 24]
        # output_layers = [24]
        # for indx, r in enumerate(self.transformer.resblocks):
        #
        #     if indx < 6 :   # -ViT-B/16   indx=6,  "CS-ViT-L/14  indx=18,
        #         x = r(x)
        #     else:
        #         x,x_ori = r(x)
        #
        ...
        for indx, r in enumerate(self.transformer.resblocks):
            x = r(x)  # 和 nn.Sequential 一样，不管 block 返回啥，直接喂进去

            # 如果需要保存中间层输出
            if (indx + 1) in output_layers:
                if isinstance(x, (list, tuple)):  # 如果 block 返回 (x, x_ori)  x_ori表示什么意思？
                    patch_tokens.append(x[0])
                    patch_tokens.append(x[1])
                    # 只取特征部分
                else:
                    patch_tokens.append(x)
                    # patch_tokens.append(x)

        # 循环结束后，模拟 "x, x_ori = self.transformer(x)" 的行为
        ...
        # patch_tokens = [self.ln_post(patch_tokens[t]) @ self.proj for t in range(len(patch_tokens))]
