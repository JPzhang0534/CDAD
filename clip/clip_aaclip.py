...
__all__ = ["available_models", "load", "tokenize", "encode_text_with_prompt_ensemble",
           "encode_text_without_prompt_ensemble",
           "get_similarity_map", "clip_feature_surgery", "similarity_map_to_points",
           "encode_text_with_prompt_ensemble_ad", 'encode_text_with_prompt_ensemble_ad_diff',
           "encode_text_with_prompt_ensemble_ad_H",
...
for cls_name in mvtec_obj_list:
    prompt_abnormal_detail[cls_name] = prompt_abnormal + ['normal {} ' + 'with {}'.format(x) for x in
                                                          mvtec_anomaly_detail[cls_name].split(',')]

for cls_name in visa_obj_list:
    prompt_abnormal_detail[cls_name] = prompt_abnormal + ['abnormal {} ' + 'with {}'.format(x) for x in
                                                          visa_anomaly_detail[cls_name].split(',')]
...
def encode_text_with_prompt_ensemble_ad(model, model_clip, objs, tokenizer, device, prompt_templates=None):
    # objs= visa_obj_list    # 换objects 为 mvtec_obj_list
    ...
    prompt_abnormal = ['broken {}', '{} with flaw', '{} with defect', '{} with damage', '{} with crack',
                       '{} with hole', '{} with scratch', 'bent {}',  # '{} with residue'     添加
...
def encode_text_with_prompt_ensemble_ad_diff(model, objs, tokenizer, device, prompt_templates=None):
    ...
    prompt_abnormal = [' {} with broken', '{} with crack', '{} with residue', '{} with damage', '{} with hole',
                       '{} with scratch",']
...
def encode_text_with_prompt_ensemble_ad_diff_word(model, objs, tokenizer, device, prompt_templates=None):
    ...
    prompted_sentence = 'a photo of {} with broken crack  damage  hole scratch flawless unblemished clear'.format(
        objs[0])  # residue 这个词有问题
