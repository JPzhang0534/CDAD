...
if __name__ == "__main__":
    ...
    def evaluate_results(results, logger, resume_path=None):
        ...
        for obj, data in grouped_results.items():
            auroc_px_list = []
            for gt_px, pr_px in zip(data["gt_px"], data["pr_px"]):
                gt_px = (np.array(gt_px) > 0).astype(np.uint8).ravel()
                pr_px = np.array(pr_px).ravel()
                auroc_px_list.append(safe_auc(gt_px, pr_px))

            auroc_px = np.nanmean(auroc_px_list)
            auroc_sp = safe_auc(np.asarray(data["gt_sp"]), np.asarray(data["pr_sp"]))
